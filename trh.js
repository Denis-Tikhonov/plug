...
//   - URL берутся AS-IS (с /function/0/ прокси)
//   - Качество определяется по суффиксу файла
...
(function () {
  ...
  function parseCards(html) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      results.push({
        ...
        img:     pic,
        poster:  pic,
        preview: pic,  // для предпросмотра в AdultJS
        time:    time,
        json:    true,
        source:  NAME,
      });
  ...
  //   Берём AS-IS — это прокси самого сайта
  // ----------------------------------------------------------
  ...
  function tryRegister() {
    if (window.AdultPlugin && typeof window.AdultPlugin.registerParser === 'function') {
      ...
      console.log('[TRH] v1.2.0 зарегистрирован');
      return true;
    }
    return false;
  }
