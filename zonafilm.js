/**
 * ============================================================================
 *  АННОТИРОВАННЫЙ ИСХОДНЫЙ КОД ПЛАГИНА «AdultJS»
 * ============================================================================
 *
 *  Назначение:
 *    Плагин для медиаплеера Lampa (Smart TV / Android / Web),
 *    предоставляющий доступ к контенту 18+ с множества сайтов.
 *    Скрипт был транслирован из ES6+ в ES5 при помощи Babel
 *    (преобразование spread-операторов, async/await, деструктуризации и т.д.).
 *
 *  Архитектура (высокоуровневая):
 *    1. PluginLogger  — простой логгер (строки 1-33)
 *    2. Babel helpers  — runtime-полифилы (45-451)
 *    3. IIFE (UI)       — регистрация в Lampa, меню, плеер, настройки (452-1048)
 *    4. HTTP-утилиты    — класс для HTTP-запросов (1049-1145)
 *    5. Модели данных   — Extractor, VideoItem, MenuItem (1146-1188)
 *    6. Скраперы сайтов — BongaCams, XVideos, XNXX, SpankBang, Chaturbate, Eporner (1189-2554)
 *    7. NextHub          — универсальный фреймворк скрапинга (2555-3319)
 *    8. Конфиг-массив    — настройки 17+ сайтов (3320-6117)
 *    9. Точка входа     — window.AdultJS (6118-6264)
 *
 *  Примечание:
 *    Все исходные комментарие сохранены без изменений.
 *    Добавлены только русскоязычные аннотации.
 * ============================================================================
 */
// ===== СЕКЦИЯ 1: МОДУЛЬ ЛОГИРОВАНИЯ (PluginLogger) =====
"use strict";
// [ВСТАВЬТЕ СЮДА МОДУЛЬ ЛОГИРОВАНИЯ]
/**
 * PluginLogger — простой модуль логирования.
 * Хранит последние 100 записей с таймстампом и данными.
 * Выводит логи в console с префиксом [AdultJS].
 */
class PluginLogger {
  constructor() {
    this.logs = [];
  ...
  /**
   * log — добавляет запись в лог. При превышении maxLogs удаляет старые.
   */
...
// ===== СЕКЦИЯ 2: BABEL RUNTIME HELPERS =====
//
// Данный блок содержит вспомогательные функции,
// сгенерированные Babel для поддержки современного
// синтаксиса JavaScript в средах, не поддерживающих его нативно:
//   - spread-оператор (...), деструктуризация
//   - async/await, генераторы
//   - for...of итерация
//   - typeof, Object.assign, классы ES6
//
// NOTE: Эти функции являются стандартным рантаймом Babel
// и не требуют ручной модификации.
//
...
// --- Регенератор генераторов (_regenerator) ---
// Обеспечивает работу async/await и function*
...
// --- Шаг async-генератора и обёртка _asyncToGenerator ---
// Превращает результат вызова генератора в Promise
//
...
// Полифил для Object.assign со свойствами-символами
// и _defineProperty — вспомогательная функция
// для безопасного определения свойств объекта.
//
...
// --- Babel-хелперы для классов ES6 ---
// _classCallCheck  — проверка вызова конструктора через new
// _defineProperties — определение методов класса
// _createClass      — полная обёртка создания класса
// _toPropertyKey   — преобразование ключа свойства
// _toPrimitive     — преобразование к примитиву
//
...
// =====================================================================
// ===== СЕКЦИЯ 3: ГЛАВНЫЙ IIFE ПЛАГИНА (UI, меню, плеер, настройки) =====
// =====================================================================
//
// Самовыполняющаяся функция (IIFE) регистрирует плагин в медиаплеере Lampa.
// Внутри создаются:
//   - Компоненты sisi_AdultJS (главная) и sisi_view_AdultJS (категория)
//   - Элемент меню «Adult JS» в главном меню Lampa
//   - Кнопка фильтра в заголовке
//   - Настройка «Предпросмотр» в разделе настроек
//
// Вложенные сущности:
//   e = "AdultJS" (имя компонента)
//   t — таймер скрытия превью
//   a — текущий элемент превью (video-wrapper)
//   n = Lampa.Reguest() — HTTP-клиент Lampa
//   o — кэш меню (массив пунктов меню)
//   l — объект-контроллер плагина (menu, view, playlist, main, search, qualitys, clear)
//   s — набор утилит (sourceTitle, play, fixCards, preview, hidePreview, fixList, menu)
//
//
!(function (e, t, a, n, r, i, o, s) {
  !(function () {
/** Имя компонента в Lampa, используется для регистрации и навигации. */
    var e = "AdultJS";
/** Локализация названия плагина (ru, en, uk, zh). */
    Lampa.Lang.add({
      lampac_adultName: {
        ru: "Adult JS",
        en: "Adult 18+",
    ...
    var t,
      ...
      n = new Lampa.Reguest();
/**
 * Вспомогательная функция r(e) — выбор ссылки нужного качества.
 * Берёт качество из настроек Lampa, если нет — первое.
 */
    function r(e) {
      var t,
...
/**
 * i() — скрытие видео-превью.
 * Останавливает воспроизведение, скрывает элемент.
 * TODO: empty catch на строке 481 — не обрабатывает ошибки при pause().
 */
    function i() {
      if ((clearTimeout(t), a)) {
        var e,
    ...
    var o,
/**
 * Объект s — набор UI-утилит плагина.
 * @property sourceTitle(e) — извлекает название сайта из домена
 * @property play(e)        — запуск видео в плеере Lampa
 * @property fixCards(e)     — нормализация карточек
 * @property preview(e, n)   — показ превью при наведении
 * @property hidePreview     — скрытие превью
 * @property fixList(e)      — нормализация списка
 * @property menu(t, a)      — контекстное меню
 */
      s = {
    ...
    /**
     * play — основной метод запуска видео в плеере.
     * Если e.json — загружает качества через qualitys(),
     * иначе — прямая ссылка.
     */
        play: function (e) {
    ...
    /** fixCards — нормализует карточки: poster/img = picture, очистка от HTML-сущностей. */
        fixCards: function (e) {
    ...
    /**
     * preview — показ видео-превью при наведении на карточку.
     * TODO: Задержка 1500мс захардкожена — вынести в настройку.
     */
        ...
        hidePreview: i,
    /** fixList — если нет quality, подставляет time. hidePreview — скрытие превью. */
        fixList: function (e) {
          return (
            e.forEach(function (e) {
    ...
    /** menu — контекстное меню (Похожие / Модель). */
        menu: function (t, a) {
          var n = [];
    ...
    /**
     * l — главный контроллер плагина (IIFE-паттерн).
     *   menu(e,t)     — загрузка списка сайтов
     *   view(e,t,a)   — загрузка страницы категории
     *   playlist(t,a,n) — параллельная загрузка плейлистов
     *   main(e,t,a)  — главная страница
     *   search(e,t,a) — поиск по всем сайтам
     *   qualitys(e,t,a) — получение качеств видео
     *   clear()        — очистка HTTP-соединений
     */
    var l = new (function () {
      var e = this,
        t = new Lampa.Reguest();
    ...
    /**
     * c(t) — создание компонента главной страницы (InteractionMain).
     * Отвечает за отображение сетки видео со всех сайтов.
     */
    function c(t) {
    ...
    /**
     * u(t) — создание компонента категории (InteractionCategory).
     * Каталог одного сайта с фильтрами, категориями и поиском.
     */
    function u(t) {
      var a,
      ...
      return (
        ...
        /** filter — меню фильтра: Найти, Сортировка, Категория. */
        (n.filter = function () {
          if (a) {
    ...
    /**
     * IIFE регистрации плагина в Lampa.
     * Создаёт элемент меню, кнопку фильтра, настройки.
     * Флаг: plugin_adultjs_AdultJS_ready (однократная инициализация).
     */
    window["plugin_adultjs_" + e + "_ready"] ||
      ...
      /** Добавление настройки «Предпросмотр» в Lampa.SettingsApi. */
...
// =====================================================================
// ===== СЕКЦИЯ 4: HTTP-УТИЛИТА (класс для выполнения запросов) =====
// =====================================================================
//
// Статический класс с методами Get и Native.
// На Android использует Lampa.Reguest.native(), иначе — fetch().
// NOTE: Таймаут запроса — 8000мс (захардкожен).
//
  var l =
      ((e = (function () {
      /** ensureHeaders — гарантирует User-Agent (Chrome 130). */
        function e() {
       ...
       * Get — HTTP GET запрос.
       * @param {string} a - URL
       * @param {Object} [n] - заголовки
       * @param {string} [r] - кодировка (TextDecoder)
       ...
       */
          ...
          {
      /**
       * Native — HTTP-запрос через нативный клиент Lampa (Android).
       * Таймаут: 8000мс.
       */
            key: "Native",
...
// =====================================================================
// ===== СЕКЦИЯ 5: МОДЕЛИ ДАННЫХ =====
// =====================================================================
// c = Extractor  — статический класс для парсинга по regex
// u = VideoItem  — элемент видео (название, URL, картинка, превью и т.д.)
// p = MenuItem  — пункт меню (название, URL плейлиста, подменю)
//
    c = (function () {
  /**
   * Extractor (c) — утилита для извлечения данных из HTML по regex.
   * @method extract(html, regex, groupIndex) — возвращает совпавшую группу или null
   */
      return _createClass(
        function e() {
          _classCallCheck(this, e);
  ...
  /**
   * VideoItem (u) — модель элемента видео.
   * @param {string} t - name, @param {string} a - video (URL),
   * @param {string} n - picture, @param {string|null} r - preview,
   * @param {string|null} i - time, @param {string|null} o - quality,
   * @param {boolean} s - json (нужен ли парсинг качеств)
   * @param {boolean} l - related, @param {Object|null} c - model
   */
  ...
  /**
   * MenuItem (p) — модель пункта меню.
   * @param {string} t - title, @param {string} a - playlist_url,
   * @param {boolean|string} [n] - search_on, @param {Array} [r] - submenu
   */
...
// =====================================================================
// ===== СЕКЦИЯ 6: СКРАПЕРЫ САЙТОВ =====
// =====================================================================
// Каждый скрапер — класс с методами:
//   Invoke(url)      — маршрутизация (список / видео)
//   Playlist(html)    — парсинг списка видео
//   Menu(...)        — меню фильтров
//   StreamLinks(html) — извлечение прямых ссылок
//
// Переменные: d=BongaCams, g=XVideos, y=XNXX, v=SpankBang, b=Chaturbate, f=Eporner
// h=StreamResult, m=QualityResult
//
    d =
    // -----------------------------------------------------------------
    // d = СКРАПЕР BongaCams (живые трансляции)
    // Host: https://ukr.bongacams.com
    // -----------------------------------------------------------------
      ((t = (function () {
        function e() {
          _classCallCheck(this, e);
        ...
        return _createClass(e, [
          {
      /** Invoke — главный метод BongaCams. Возвращает {menu, list}. */
            key: "Invoke",
            value:
              ((t = _asyncToGenerator(
          ...
          {
      /**
       * Playlist — парсинг BongaCams HTML.
       * Ищет блоки моделей, формирует HLS-ссылки.
       */
            key: "Playlist",
            value: function (e) {
              var t = [];
          ...
          {
      /** Menu — меню BongaCams: Новые, Пары, Девушки, Русские, Парни, Транс. */
            key: "Menu",
            value: function (t) {
              var a = e.host + "/",
                n = [
      ...
      t),
  /** h = StreamResult — модель результата страницы видео. */
  ...
  /** m = QualityResult — модель результата qualitys(). */
    ...
    g =
      ...
      /** Invoke — главный метод XVideos. */
          ...
          {
      /** buildUrl — построение URL: поиск, категории, топ, новые. */
            key: "buildUrl",
          ...
          {
      /** getLastMonth — предыдущий месяц YYYY-MM для URL топа. */
            key: "getLastMonth",
            value: function () {
              var e = new Date();
              return (
          ...
          {
      /**
       * Playlist — парсинг HTML XVideos.
       * NOTE: Сложная трансформация URL превью.
       */
            key: "Playlist",
            value: function (t) {
              if (!t) return [];
          ...
          {
      /** Menu — меню XVideos (поиск + сортировка + 30+ категорий). */
            key: "Menu",
            value: function (t, a) {
              var n,
                r = e.host,
          ...
          {
      /**
       * StreamLinks — извлечение HLS и похожих видео.
       * TODO: try/catch на стр. 1627 может падать при изменении формата.
       */
            key: "StreamLinks",
            value: function (t) {
    ...
    // Аналогичен XVideos, но с другими селекторами.
    // -----------------------------------------------------------------
    y =
      ((n = (function () {
        function e() {
    ...
    // Поддержка множества качеств (2160p–360p).
    // -----------------------------------------------------------------
    v =
      ((r = (function () {
        function e() {
        ...
        return _createClass(e, [
          ...
          {
      /**
       * StreamLinks — извлечение MP4-ссылок.
       * Суффикс "k" (как 4k) преобразуется в 2160.
       */
            key: "StreamLinks",
            value: function (e) {
              for (
      ...
      r),
    // -----------------------------------------------------------------
    // b = СКРАПЕР Chaturbate (живые трансляции)
    // Host: https://chaturbate.com
    // API: /api/ts/roomlist/room-list/
    // -----------------------------------------------------------------
    b =
      ((i = (function () {
        function e() {
        ...
        return _createClass(e, [
          ...
          {
      /**
       * buildUrl — URL для API Chaturbate.
       * Лимит: 90 результатов. Пагинация: offset = 90 * page.
       */
            key: "buildUrl",
            value: function (e, t, a) {
              var n =
          ...
          {
      /** StreamLinks — получение HLS-ссылки Chaturbate (загрузка страницы модели). */
            key: "StreamLinks",
            value:
              ((t = _asyncToGenerator(
                _regenerator().m(function t(a) {
    ...
    // Использует XHR API: /xhr/video/ для получения MP4-ссылок.
    // -----------------------------------------------------------------
    f =
      ((o = (function () {
        function e() {
        ...
        return _createClass(e, [
          ...
          {
      /**
       * StreamLinks — получение качеств Eporner.
       * Извлекает vid/hash, запрашивает /xhr/video/.
       */
            key: "StreamLinks",
            value:
              ((t = _asyncToGenerator(
          ...
          {
      /**
       * convertHash — конвертация hash Eporner.
       * 32 hex -> 4 части по 8 символов, каждая hex->base36.
       */
            key: "convertHash",
            value: function (e) {
              return (
          ...
          {
      /** base36 — конвертация hex в base36. */
            key: "base36",
            value: function (e) {
...
// =====================================================================
// ===== СЕКЦИЯ 7: NEXTHUB — УНИВЕРСАЛЬНЫЙ ФРЕЙМВОРК СКРАПИНГА =====
// =====================================================================
// NextHub позволяет описывать скраперы в виде конфигурационных объектов.
//
  function k(e, t) {
/** k(e,t) — замена шаблонов {key} на значения. */
  ...
  function w(e, t) {
/** w(e,t) — безопасная склейка пути (удаление дублирующихся слэшей). */
    var a = e.replace(/\/+$/, ""),
      n = t.replace(/^\/+/, "");
    return a + (n ? "/" + n : "");
  }
  function _(e) {
/** _(e) — парсинг HTML в DOM через DOMParser. */
    return new DOMParser().parseFromString(e, "text/html");
  }
  function x(e, t, a) {
/** x(e,t,a) — XPath-запрос (первый элемент). */
    return e.evaluate(
      t,
      a || e,
      null,
  ...
  function C(e, t, a) {
/** C(e,t,a) — получение атрибута. Поддерживает массив атрибутов. */
    if (!e) return "";
    if (Array.isArray(t)) {
      var n,
  ...
  var S =
  /**
   * S = NextHubScraper — класс универсального скрапера.
   * Методы: buildListUrl, buildSearchUrl, buildModelUrl,
   ...
   */
      ((s = (function () {
        return _createClass(
          ...
          [
            {
              /** buildListUrl — построение URL списка. Учитывает firstpage, route, шаблоны. */
              key: "buildListUrl",
              value: function (e, t, a, n) {
                var r,
                  i,
            ...
            {
              /** buildSearchUrl — построение URL поиска. */
              key: "buildSearchUrl",
              value: function (e, t, a) {
            ...
            {
              /** buildModelUrl — построение URL модели. */
              key: "buildModelUrl",
              value: function (e, t, a) {
                var n,
                  r =
            ...
            {
              /** buildMenu — создание MenuItem[]: Поиск, Похожие, Сортировка, Категории. */
              key: "buildMenu",
              value: function (e, t, a) {
                var n,
                  r,
            ...
            {
              /**
               * toPlaylist — парсинг DOM в VideoItem[].
               * NOTE: Обработка относительных URL обложек.
               */
              key: "toPlaylist",
              value: function (e, t) {
                var a,
            ...
            {
              /**
               * extractStreams — извлечение прямых ссылок.
               * Стратегии: iframe, eval, nodeFile, regexMatch.
               * NOTE: eval использует new Function() — потенциальная уязвимость.
               */
              key: "extractStreams",
              value:
                ((t = _asyncToGenerator(
            ...
            {
              /**
               * Invoke — маршрутизация NextHub.
               * Режимы: view/related, model, search, list.
               * URL: nexthub://DisplayName?mode=list&sort=...&cat=...&pg=...
               */
              key: "Invoke",
              value:
                ((e = _asyncToGenerator(
...
// =====================================================================
// ===== СЕКЦИЯ 8: КОНФИГУРАЦИОННЫЙ МАССИВ САЙТОВ =====
// =====================================================================
//
// Массив P — конфиги для универсального скрапера NextHub.
//
// Структура объекта:
//   enable, displayname, host, charset,
//   menu: { route: {sort, cat, catsort, model}, sort: {}, categories: {} },
//   list: { uri, firstpage? }, search: { uri },
//   contentParse: { nodes, name, href, img, preview, duration, model? } — XPath
//   view: { related?, iframe?, eval?, nodeFile?, regexMatch? } — правила извлечения ссылок
//
    P = [
      ...
      // --- NoodleMagazine (adult.noodlemagazine.com) ---
      {
        enable: !0,
      ...
      // --- SemBatsa (sem.batsa.pro) ---
      // NOTE: enable: false — сайт отключён
      {
        enable: !1,
      ...
      // --- GayPornTube (www.gayporntube.com) ---
      {
        enable: !0,
      ...
      // --- Vtrahe (site.vtrahehd.tv) ---
      // NOTE: charset: windows-1251, view.eval — JS-код для URL видео
      {
        enable: !0,
        displayname: "Vtrahe",
      ...
      // --- VtraheTV (my.vtrahe.work) ---
      // NOTE: view.eval — JS-код для URL видео
      {
        enable: !0,
      ...
      // --- TrahKino (trahkino.me) ---
      // NOTE: firstpage: "{host}" — особый URL для первой страницы
      {
        enable: !0,
...
// =====================================================================
// ===== СЕКЦИЯ 9: ТОЧКА ВХОДА — ИНИЦИАЛИЗАЦИЯ И ROUTER =====
// =====================================================================
//
// Экземпляры скраперов и глобальный объект window.AdultJS.
//
    z = new d(),
  // z = BongaCams
    L = new g(),
  ...
  // A = Eporner
    I = new S(P);
  // I = NextHub (с конфигом P)
  !(function () {
    function e() {
/**
 * Центральный роутер запросов.
 * Последовательно проверяет URL на совпадение с хостами скраперов.
 * Порядок: BongaCams -> XVideos -> XNXX -> SpankBang ->
 * Chaturbate -> Eporner -> nexthub:// -> конфиг P -> "unknown site"
 */
    ...
    /**
     * window.AdultJS — глобальный API плагина.
     * @property Menu() — массив пунктов меню
     * @property Invoke(url) — роутинг запросов
     */
    window.AdultJS = {
      Menu: function () {
        // 6 встроенных скраперов + NextHub-сайты
        var e = [
