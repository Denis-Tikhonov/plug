...
(function () {
  ...
  function parsePlaylist(html) {
    ...
    items.forEach(function (el) {
      ...
      var name = (titleEl ? (titleEl.getAttribute('title') || titleEl.textContent) : a.textContent).replace(/\s+/g, ' ').trim() || 'Video';
      var durEl = el.querySelector('.duration');
      var time = durEl ? durEl.textContent.trim() : '';
      results.push({ name: name, video: href, picture: pic, img: pic, poster: pic, background_image: pic, time: time, quality: 'HD', json: true, source: NAME });
    });
  ...
  function buildMenu() {
    return [
      ...
      { title: '📂 Категории', playlist_url: 'submenu', submenu: CATEGORIES.filter(function (c) { return c.slug; }).map(function (c) { return { title: c.title, playlist_url: NAME + '/cat/' + c.slug }; }) },
    ];
  }
